...
export const handleStackableValidationResult = (validationValidateStackableResponse: ValidationValidateStackableResponse) => {
    if (!validationValidateStackableResponse?.inapplicable_redeemables) {
        return;
    }

    const inapplicableRedeemableResult = validationValidateStackableResponse.inapplicable_redeemables.find(redeemable => !!redeemable?.result?.error)?.result;
